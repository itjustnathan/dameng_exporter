...
func InitLogger(logFile string) {
	...
	logFileName := fmt.Sprintf(logFile) // 日志文件路径

	// 配置日志切割器
	lumberjackLogger := &lumberjack.Logger{
		Filename:   logFileName, // 日志文件路径
		MaxSize:    50,          // 每个日志文件最大为 50 MB
		MaxBackups: 5,           // 保留最近的 5 个日志文件备份
	...
	// 创建 zap 核心
	core := zapcore.NewTee(
		zapcore.NewCore(consoleEncoder, zapcore.NewMultiWriteSyncer(fileWriter), zapcore.DebugLevel),
		zapcore.NewCore(consoleEncoder, zapcore.NewMultiWriteSyncer(consoleWriter), zapcore.InfoLevel),
	)
	...
	Logger = logger.Sugar()
	Logger.Infof("Log information will be logged: %s", logFileName)
